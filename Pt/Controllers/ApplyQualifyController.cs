...
{
    [Authorize]
    public class ApplyQualifyController : PkpmController
    {
        ...
        // GET: CheckQualify
        public ApplyQualifyController(ISysDictService sysDictServcie,
            ICheckUnitService checkUnitService,
            IRepsitory<t_D_UserTableOne> repOne,
            IRepsitory<t_D_UserTableTwo> repTwo,
             IRepsitory<t_D_UserTableFive> repFive,
        ...
        public ActionResult Index()
        {
            ...
            viewModel.CompanyStatus = sysDictServcie.GetDictsByKey("qualifyStatus");
        ...
        public ActionResult ToolBar()
        {
            ...
            {
                toolBar.AddToolbarItem(new DhtmlxToolbarButtonItem("NewCustom", "新增") { Img = "fa fa-plus", Imgdis = "fa fa-plus" });
                toolBar.AddToolbarItem(new DhtmlxToolbarSeparatorItem("1"));
            }
        ...
        public ActionResult Search(ApplyQualifySearchModel searchModel)
        {
            ...
            for (int i = 0; i < data.Results.Count; i++)
            {
                ...
                //判断是否是检测机构角色 机构本身可以 添加/编辑/查看
                var tbTwo = repTwo.GetByCondition(r => r.pid == Convert.ToInt32(custom.ID)).FirstOrDefault();
                var tbThree = repThree.GetByCondition(r => r.pid == custom.ID.ToString()).FirstOrDefault();
                var tbFour = repFour.GetByCondition(r => r.pid == custom.ID.ToString()).FirstOrDefault();
                var tbFive = repFive.GetByCondition(r => r.pid == custom.ID.ToString()).FirstOrDefault();
                var tbSix = repSix.GetByCondition(r => r.pid == custom.ID.ToString()).FirstOrDefault();
                var tbSeven = repSeven.GetByCondition(r => r.pid == custom.ID.ToString()).FirstOrDefault();
                if (currentUserRole.Code == "JCZXYH" && custom.@static < 2)
                {
                    row.AddLinkJsCell(custom.UNITNAME + "/" + custom.NAME + "[编辑]", "EditQualifyOne(\"{0}\")".Fmt(custom.ID)); //单位名称/申请人姓名
                    if (tbTwo != null)
                    {
                        row.AddLinkJsCell("[编辑]", "EditQualifyTwo(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE)); //机构基本情况表
                    }
                    else
                    {
                        row.AddLinkJsCell("[添加]", "AddQualifyTwo(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE)); //机构基本情况表
                    }

                    if (tbThree != null)
                    {
                        row.AddLinkJsCell("[编辑]", "EditQualifyThree(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE)); //法定代表人基本情况
                    }
                    else
                    {
                        row.AddLinkJsCell("[添加]", "AddQualifyThree(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE)); //法定代表人基本情况
                    }

                    if (tbFour != null)
                    {
                        row.AddLinkJsCell("[编辑]", "EditQualifyFour(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE)); //技术负责人基本情况
                    }
                    else
                    {
                        row.AddLinkJsCell("[添加]", "AddQualifyFour(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE)); //技术负责人基本情况
                    }

                    row.AddLinkJsCell("[编辑]", "EditQualifyFive(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE));//检测类别、内容、具备相应注册工程师资格人员情况


                    row.AddLinkJsCell("[编辑]", "EditQualifySix(\"{0}\")".Fmt(custom.ID)); //专业技术人员情况总表


                    row.AddLinkJsCell("[编辑]", "EditQualifySeven(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE)); //主要仪器设备（检测项目）及其检定/校准一览表
                }
                else
                {
                    row.AddLinkJsCell(custom.UNITNAME + "/" + custom.NAME + "[查看]", "DetailQualifyOne(\"{0}\")".Fmt(custom.ID));
                    if (tbTwo != null)
                    {
                    ...
                    row.AddLinkJsCell("[查看]", "DetailQualifyFive(\"{0}\",\"{1}\")".Fmt(custom.ID, custom.UNITCODE));//检测类别、内容、具备相应注册工程师资格人员情况
                ...
                //状态
                row.AddCell(sysDictServcie.GetDictsByKey("qualifyStatus", custom.@static.ToString()));

                if (HaveButtonFromAll(buttons, "Audit") && custom.@static == 1)
                {
                    row.AddLinkJsCell("受理", "Audit(\"{0}\")".Fmt(custom.ID));
                }
                else if (HaveButtonFromAll(buttons, "Print") && custom.@static == 4)
                {
                    row.AddLinkJsCell("打印通知单", "Print(\"{0}\")".Fmt(custom.ID));
                }
                else if (custom.@static == 5)
                {
                    var tbTen = repTen.GetByCondition(r => r.pid == custom.ID).FirstOrDefault();
                    if (tbTen != null)
                    {
                        row.AddLinkJsCell("查看原因", "Reason(\"{0}\")".Fmt(tbTen.outstaticinfo));
                    }
                    else
                    {
                        row.AddCell(string.Empty);
        ...
        private SearchResult<ApplyQualityUIModel> GetSearchResult(ApplyQualifySearchModel searchModel)
        {
            ...
            predicate = predicate.And(t => t.type == "0");
            ...
            {
                predicate = predicate.And(t => t.@static == Convert.ToInt32(searchModel.status));
            }
        ...
        public ActionResult UnitQualification(string id, string isSaveButton)
        {
            ...
            foreach (var QualiFications in sysDictServcie.GetDictsByKey("unitQualifications"))
            {
        ...
        public ActionResult Delete(int id)
        {
            ...
            try
            {
                ...
                {
                    LogUserAction("对检测机构法定代表人声明ID为{0}进行了删除操作".Fmt(id));
                }
            ...
            try
            {
                ...
                foreach (var id in selectedId.Split(','))
                {
                    ...
                    if (tbOne.@static != 0 || 
                        tbOne.onepath_zl != "1" ||
                        tbOne.twopath_zl != "1" ||
                        tbOne.threepath_zl != "1" ||
                        tbOne.Fourpath_zl != "1" ||
                        tbOne.fivepath_zl != "1" ||
                        tbOne.Sixpath_zl != "1" ||
                        tbOne.Sevenpath_zl != "1")
                    {
                        result = ControllerResult.FailResult;
                        result.ErroMsg = "记录当前状态不能进行递交操作，请填写完整信息才能递交";
                        return Content(result.ToJson());
                    }
                }

                if (!checkQualifyService.SetInstSendState(selectedId, state, "t_D_UserTableOne", out erroMsg))
                {
                    result = ControllerResult.FailResult;
                    result.ErroMsg = erroMsg;
                }
        ...
        {
            var tbOne = repOne.GetById(id);
            var tbTen = repTen.GetByCondition(r => r.pid == id).FirstOrDefault();
            var model = new QualifyPrintUIModel()
            {
                unitname = tbOne.unitname,
                time = DateTime.Now,
                EndTime = tbTen != null ? tbTen.ThreeTime.Value.ToString("yyyy年MM月dd日") : DateTime.Now.ToString("yyyy年MM月dd日")
            };

            model.ImageUrl = CreateQrcode(tbOne.unitCode);
        ...
        private ApplyQualifyTwoUIModels GetApplyQualifyTwoDetailViewModel(string id, string customid)
        {
            ...
            if (viewModel.myT_D_UserTableTwo != null)
            {
        ...
        [ValidateInput(false)]
        [HttpPost]
        public ActionResult Edit(ApplyCustomSaveViewModel viewModel)
        {
            ControllerResult result = ControllerResult.SuccResult;
            ApplyCustomSaveModel editModel = new ApplyCustomSaveModel();
            try
            {
                editModel.OpUserId = GetCurrentUserId();
                DateTime? detectstartDateTime = null;
                DateTime? detectendDateTime = null;
                ...
                #region 
                editModel.custom = new t_bp_custom_apply()
                {
                    ID = viewModel.ID,
                    NAME = viewModel.NAME,
                    ...
                    SQname = viewModel.SQname,
                    SQTel = viewModel.SQTel,
                    sqjcyw = viewModel.sqjcyw,
                    FRSex = viewModel.FRSex,
                    FRBirth = viewModel.FRBirth,
                    FRTITLE = viewModel.FRTITLE,
                    FRYEAR = viewModel.FRYEAR,
                    FRGraduationTime = viewModel.FRGraduationTime,
                    FRCollege = viewModel.FRCollege,
                    FREducation = viewModel.FREducation,
                    FRSubject = viewModel.FRSubject,
                    FRTel = viewModel.FRTel,
                    FRMobile = viewModel.FRMobile,
                    JSSex = viewModel.JSSex,
                    JSBirth = viewModel.JSBirth,
                    JSGraduationTime = viewModel.JSGraduationTime,
                    JSCollege = viewModel.JSCollege,
                    JSEducation = viewModel.JSEducation,
                    JSSubject = viewModel.JSSubject,
                    JSTel = viewModel.JSTel,
                    JSMobile = viewModel.JSMobile,
                    bgcszmPath = viewModel.bgcszmPath,
                    zxzmPath = viewModel.zxzmPath,
                    gqzmPath = viewModel.gqzmPath,
                    zzscwjPath = viewModel.zzscwjPath,
                    frzcpath = viewModel.frzcpath,
                    glscwjPath = viewModel.glscwjPath,
                    frxlpath = viewModel.frxlpath,
                    jszcpath = viewModel.jszcpath,
                    jsxlpath = viewModel.jsxlpath
                };
                #endregion

                if (viewModel.frWorkExperience != null)
                {
                    editModel.custom.frgzjl = viewModel.frWorkExperience.Select(r => r.WorkExperienceTime + "&" + r.WorkExperienceContent).ToList().Join("|");
                }
                if (viewModel.jsWorkExperience != null)
                {
                    editModel.custom.jsgzjl = viewModel.jsWorkExperience.Select(r => r.WorkExperienceTime + "&" + r.WorkExperienceContent).ToList().Join("|");
                }
        ...
        public ActionResult EditQualifyOne(int id)
        {
            ...
            if (oneQualifyModel != null)
            {
                ...
                model.AddTime = oneQualifyModel.addtime;
            }
